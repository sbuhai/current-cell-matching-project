...
def _write_grid_candidate_table() -> None:
    ...
    lines.extend(
        [
            ...
            r"}",
            r"\begin{minipage}{0.99\textwidth}",
            r"\footnotesize Notes. Panel B fixes the tolerance band. The candidate support share is computed on the graph admitted by that band, not on the target domain alone. $\widehat{\Spread}$ is the finite log spread of the recovered multipliers within a fiber; $\widehat{\DemDist}$ is the corresponding transport distance across ages. The cap $K$ is the first value at which the search has reached all retained pairs. The maintained row sets $\varepsilon=0.005$.",
            r"\end{minipage}",
            r"\end{table}",
            "",
        ]
